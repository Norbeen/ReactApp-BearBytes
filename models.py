...
class reviewPost(db.Model):
    ...
    def __repr__(self):
        ...
        return "{'food comment':'%s', 'food rating':'%s', 'menu id':'%s', 'food like':'%s', 'food dislike':'%s', 'food author':'%s', 'food image':'%s' , 'food date':'%s'}" % (self.Ucomment, self.Urating, self.UmenuItemId, self.Ulike, self.Udislike, self.Uauthor,self.Uimage , self.Udate)
